...
class PSABlock(nn.Module):
    ...
    def __init__(self, c, attn_ratio=0.5, num_heads=4, shortcut=True) -> None:
        ...
        # self.attn = DeformableAttention(embed_dim=c, num_heads=num_heads, num_levels=4, num_points=4)
        """
            Args:
            embed_dim(int): 输入特征的维度
            num_heads(int): 多头注意力中的头数
            num_levels(int): 特征图的层数（多尺度支持）
            num_points(int): 每个query的采样点数
        """
...
class SPP_CA(nn.Module):
    """融合SPPF多尺度池化与CA注意力机制的新模块"""
    ...
    def __init__(self, c1, c2, e=0.5, k=5):
        ...
        # self.ema = EMA(c_ * 4 )
        self.c2psa = C2PSA(c2, c2, n=3, e=0.5)
        # self.cvlk = Conv(c2, c_, 1, 1)
        # self.lkst =  LSKA_ATT(c_,k_size=7)
        # self.lkst2 = LSKA_ATT(c_,k_size=11)
        # self.lkst3 = LSKA_ATT(c_,k_size=23)
        # self.lkst4 = LSKA_ATT(c_,k_size=35)
    ...
    def forward(self, x):
        ...
        # t = self.cv2(self.ema(torch.cat(y , 1)))
        # t = self.deform(t)
        # print(t.shape)
        return t
...
class SPPF_LSKA_EMA(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.lska = LSKA(c_ * 4, k_size=11)
        # self.corr = CoordAtt(c2,c2)
        self.ema = EMA(c2)
        # self.c2psa = C2PSA(c2, c2)
    ...
    def forward(self, x):
        ...
        y1 = self.m(x)
        y2 = self.m(y1)
        t = self.cv2(self.lska(torch.cat((x, y1, y2, self.m(y2)), 1)))
        # t = self.cv2(torch.cat((x, y1, y2, self.m(y2)), 1))
        # t = self.corr(t)
        t = self.ema(t)
        # t = self.c2psa(t)
        return t
...
class DynamicCoordConv(nn.Module):
    """ 动态坐标卷积（DCC）模块 """

    def __init__(self, c1, c2, k=3, reduction=16):
        super().__init__()
        ...
        self.conv = nn.Conv2d(c1, c2, k, padding=k // 2, groups=4)
    ...
    def forward(self, x):
        ...
        dynamic_weights = y * self.conv.weight
        x = F.conv2d(x, dynamic_weights, self.conv.bias,
                     padding=self.conv.padding, groups=4)
        return self.ca(x)

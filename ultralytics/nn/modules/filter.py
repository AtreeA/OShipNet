...
class DynamicFilter(nn.Module):
    def __init__(self, dim, expansion_ratio=2, reweight_expansion_ratio=.25,
        ...
        # print(size[1])
        self.size = size[0] # 14
        self.filter_size = size[1] // 2 + 1 # 8
        # print(self.filter_size)
        self.num_filters = num_filters  # 4
        self.dim = dim
        self.med_channels = int(expansion_ratio * dim)
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 3)     # (B, C, H, W) -> (B, H, W, C)
        # print(x.dtype)
        B, H, W, _ = x.shape
        ...
        # complex_weights = complex_weights.to(torch.complex64)
        # print(routeing.shape)
        # complex_weights = complex_weights.to(torch.complex64)
        # print(routeing.dtype, complex_weights.dtype)
        routeing = routeing.to(torch.complex64)
        if complex_weights.dtype != routeing.dtype:
            complex_weights = complex_weights.to(routeing.dtype)

        weight = torch.einsum('bfc,hwf->bhwc', routeing, complex_weights)
        # print("weights1",weight.shape)
        if self.weight_resize:
        ...
        with autocast():
            x = self.pwconv2(x)
        # print("output after dynamic filter shape: ", x.shape,'\n')
...
if __name__ == '__main__':
    model = DetDFFormerM36()
    # model = DynamicFilter(128)
    # model = dfformer_m36()

...
from .EMA import EMA
from .waveconv import WTConv2d
from einops import rearrange

__all__ = ['MetaNeXtStage', 'MetaNeXtBlock', 'InceptNeXtStage']
...
class InceptionDWConv2d(nn.Module):
    def __init__(self, in_channels, square_kernel_size=3, band_kernel_size=11, branch_ratio=0.125):
        ...
        self.dwconv_hw = nn.Conv2d(gc, gc, square_kernel_size, padding=square_kernel_size // 2,
                                   groups=gc)
        self.dwconv_w = nn.Conv2d(gc, gc, kernel_size=(1, band_kernel_size), padding=(0, band_kernel_size // 2),
                                  groups=gc)
        self.dwconv_h = nn.Conv2d(gc, gc, kernel_size=(band_kernel_size, 1), padding=(band_kernel_size // 2, 0),
                                  groups=gc)
        self.RFA = RFAConv(2 * gc, 2 * gc, kernel_size=3)
        self.wave = WTConv2d(2 * gc, 2 * gc, kernel_size=3)
        self.split_indexes = [gc, gc, gc, gc, 2 * gc, 2 * gc]
        self.act = nn.GELU()

    def forward(self, x):
        cut = x
        x_id, x_hw, x_w, x_h, x_rfa, x_wave = torch.split(x, self.split_indexes, dim=1)
        x = torch.cat(
            (x_id, self.dwconv_hw(x_hw), self.dwconv_w(x_w), self.dwconv_h(x_h), self.RFA(x_rfa), self.wave(x_wave)),
            dim=1,
        )
        x = self.act(x)
        return x + cut
...
class InceptNeXtStage(nn.Module):
    def __init__(
            self, in_chs, out_chs, depth=2, ds_stride=2,
            drop_path_rates=None, ls_init_value=1.0,
        ...
        stage_blocks = []
        self.conv = nn.Conv2d(in_chs, out_chs, kernel_size=3, padding=1, stride=2)
...
class MetaNeXtStage(nn.Module):
    def __init__(
            self, in_chs, out_chs, depth=2, ds_stride=2, drop_path_rates=None, ls_init_value=1.0,
            token_mixer=InceptionDWConv2d, act_layer=nn.GELU, norm_layer=nn.BatchNorm2d, mlp_ratio=4,
    ):
        super().__init__()
...
if __name__ == '__main__':
    stage0 = MetaNeXtStage(in_chs=3, out_chs=40, depth=2)
    stage1 = MetaNeXtStage(in_chs=40, out_chs=80, depth=2)
    stage2 = MetaNeXtStage(in_chs=80, out_chs=160, depth=6)
    stage3 = MetaNeXtStage(in_chs=160, out_chs=320, depth=2)

...
class FocalLoss(nn.Module):
    ...
    @staticmethod
    def forward(pred, label, gamma=0.5, alpha=0.25):
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, OIoU=True,scale= True)
...
class EMASlideLoss:
    ...
    def decay(self, x):
        # 提取lambda函数为常规函数
        return self.decay_factor * (1 - math.exp(-x / self.tau))

    def __call__(self, pred, true, auto_iou=0.5):
        if self.is_train and auto_iou != -1:
            self.updates += 1
            d = self.decay(self.updates)  # 使用提取后的 decay 方法
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.EMASLoss = EMASlideLoss(nn.BCEWithLogitsLoss(reduction='none'))  # Exponential Moving Average Slide Loss
    ...
    def __call__(self, preds, batch):
        ...
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl, dice
        ...
        loss[1] = self.EMASLoss(pred_scores, target_scores.to(dtype), auto_iou).sum() / target_scores_sum
        ...
        loss[0] *= self.hyp.box  # box gain    self.hyp.box = 7.5
        loss[1] *= self.hyp.cls  # cls gain    self.hyp.cls = 0.5
        loss[2] *= self.hyp.dfl  # dfl gain    self.hyp.cls = 1.5

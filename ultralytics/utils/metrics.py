...
def bbox_iou(box1, box2, xywh=True,
    ...
    if scale:
        self = Scale(1 - (inter / union))

    iou = torch.pow(inter / (union + eps), alpha)  # alpha iou
    if CIoU or DIoU or GIoU or EIoU or SIoU or OIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or OIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:
                ...
                if Focal:
                    ...
                    ciou = iou - (rho2 / c2 + torch.pow(v * alpha_ciou + eps, alpha))
                ...
                F1 = (d12 + d22) / (cw2 + ch2)
                iou1 = getattr(Scale, '_scaled_loss')(self)
                iou2 = (1 - iou) * torch.exp((rho2 / c2))
                F2 = (iou1 * iou2).mean()
                return iou - lamda * F1 - miu * F2
...
def plot_pr_curve(px, py, ap, save_dir=Path("pr_curve.png"), names={}, on_plot=None):
    ...
    ax.plot(px, py.mean(1), linewidth=3, color="blue", label=f"all classes")
